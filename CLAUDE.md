...
mt2_collect_observations.py → guided TUI collector → difficulty_observations.csv (Bloom/Tangle/Overgrowth enemy AND boss ATK/HP + notes + verified). Entering a stat in the run-walk sets verified=Yes (the act of observing in-game); bulk/seed modes write verified=No. The walk keys its ✓/remaining on verified (not just on having stats), so the marks are: ✓ verified · ~ provisional (seeded ATK/HP, unconfirmed — shown "(unverified)") · blank unrecorded. Pressing Enter on a ~ row accepts its prefilled value and flips it to ✓. Maintenance modes: --check (validator, incl. NOTE divergence), --prefill-base DIFF (seed a difficulty's non-boss cells from roster base, verified=No), --tidy-notes (propagate each enemy's note across its orders/difficulties), --seed-notes SRC DST (copy SRC's notes onto DST as notes-only rows with blank ATK/HP — e.g. `--seed-notes Bloom Tangle` so a fresh Tangle run shows each enemy's note for inline editing), --seed-bosses-from-computed (bridge-seed 84 Overgrowth boss rows from the computed BOSS_STATS in gamedata.js, verified=No, to be observed-and-overwritten). See its module docstring.
