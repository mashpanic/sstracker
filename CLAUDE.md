...
**Boss is at the BACK of its wave (fixed 2026-06-14).** In-game the boss spawns at the back of the final wave. Display order is storage-reversed, which only lands a boss at the back when it was stored at the *head* (single-copy minor boss, e.g. Korin → correct). The designated boss is appended at the *tail* of storage (doubled in `BossBattle`; single-copy Astrael), so the reversal floated it to the **front** (e.g. Maera O3 ⑥ showed `Maera, Sentinel, Sentinel` but Maera is actually at the back). Fix: after dedup, `parse_scenario` moves all `is_boss` enemies to the end of the final wave (idempotent for the head-stored case). All region bosses, Astrael and Lifemother now render last in their wave.
...
- [ ] **Minor boss ↔ wave set is NOT a fixed pairing (verified wrong).** The project assumed each minor boss locks to one wave set (the `Minor boss → wave set` table in Game knowledge, the "Wave-set → scenario mapping" list below, and the pairing dicts in `mt2_emit_wave_descriptions.py`). **That's false** — per the user's experience and the Tivi's Region wiki page, in R3 *either* Phalanx *or* The Undying Spirit can front *either* Harassing Snipers *or* Rabble-Rousers; boss and trash are chosen independently. **Action:** (a) verify the same independence for the other paired-minor regions — R1 (Athane/Korin), R2 (Elebor/Quoto), R4 (Qel/Ajax) — against the wiki + play; (b) stop baking the boss into the wave-set string (the final-wave boss name is currently hard-coded per wave set in `WAVE_SET_DESCRIPTIONS`); instead drive the displayed final-wave boss from the selected **battle variant** so any boss×wave-set combination renders correctly. This **subsumes** the alt-boss-name item below (Quoto→Elebor / Ajax→Qel are just one symptom of the same baked-in-boss bug). The app UI already lets you pick boss and wave set independently (separate selects in the battle row), so only the displayed wave string is wrong. **Re-check the central wave data assumption in `mt2_extract_waves.py` too** — whether each `Battle` scenario's designated boss is really the boss or just one possibility.
- [ ] **Alt-boss wave shows the *sibling's* name in the app.** (Subsumed by the pairing TODO above — kept for the concrete example.) Because Quoto reuses Elebor's wave set (`Gluttonous Masses`) and Ajax reuses Qel's (`Plague Legion`), the shared wave string's final wave names the *sibling* — so selecting **Quoto** in the R2 battle shows the boss wave ending in **Elebor the Unstoppable** (and Ajax → **Qel the Malaiser**). Cosmetic/confusing only; the boss's own stats are correct. **Fix:** when the selected battle variant is the alt boss, swap the sibling's name for the alt's in the displayed final wave (e.g. in `wrapEnemyStats`/`getDisplayText`, or via an alt→sibling map), or carry the alt's name in the wave data.
- [ ] **Auto-select the wave set when only one exists for a battle.** Battle rows have two dropdowns — the minor-boss (battle variant) select and the wave-set select. When a region has only a single wave-set option, picking the boss should auto-populate the wave-set select instead of requiring a second manual pick (only relevant to minor bosses / `*-battle` rows). Implementation notes: wave-set options come from `WAVE_SET_OPTIONS[region]` in `gamefacts.js` (populated by `populateWaveSetSelects()` in `app.js`); relevant selects are `#<region>-wave-set` and `#<region>-battle-variant`. Natural hook is `handleVariantChange()` in `app.js` — when a battle variant is chosen, if `WAVE_SET_OPTIONS[region]` has exactly one entry, set `#<region>-wave-set` to it, call `updateNodeDisplay` on it, refresh the info box, and `saveState`. Watch the shared-wave-set cases (e.g. R2's `Gluttonous Masses` fronts both Elebor and Quoto; the minor boss ↔ wave-set pairing isn't strictly fixed) — judge "only one wave set" by the *count of options for the region*, not per boss.
