...
> **Wave-set names are a GAME FACT** = each scenario's `ScenarioData.battleNameKey`, the banner shown on battle entry (resolve via `mt2_lib.Localizer`). Verified they match `WAVE_SET_OPTIONS` exactly: Favored Ascent, Dutiful Sentinels, Gluttonous Masses (`Forbidden Fruit` = the excluded R2 TroopTitanskin), Harassing Snipers, Rabble-Rousers, Plague Legion. So `WAVE_SET_OPTIONS` is **extractable** from `battleNameKey` instead of hand-authored. **Exception — Astrael (R0):** both its wave sets (`Infested`, `Morsels`) banner as **"Astrael, the First Reborn"** (the boss name), so the game gives them no distinct name — the tracker's two Astrael options ("Mother's Flagellant" / "Mother's Hunter", named after each set's signature enemy) are a **tracker-only** distinction, not in-game banners. (BossBattle scenarios' `battleNameKey` is the boss name, not a wave-set name.)
