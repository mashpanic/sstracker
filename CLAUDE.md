...
- [ ] **Audit `waves.json` against `ENEMY_STATS` (coverage gap).** Cross-check every enemy internal that spawns in `out/waves.json` against the internals present in the Overgrowth rows of `difficulty_observations.csv` (what `ENEMY_STATS`/`ENEMY_NOTES` emit from). Any spawning internal with **no Overgrowth rows** is missing from `ENEMY_STATS`, so it's not in the app's name regex → renders as **plain text in wave lists** (no inline stats, no note popover). This is the base-vs-upgraded trap: a *base* enemy that appears only at low Overgrowth orders (e.g. `EnchanterT1_Speed` / **Energizing Flautist** in Favored Ascent O1/O2, found 2026-07-03) is easy to miss because its upgraded `_Ver2` twin covers the higher orders. Fix each gap by adding the enemy's Overgrowth rows (note + stats; blank stats + `verified=No` until observed). Script it: collect spawning internals per order from `waves.json` (respecting Astrael=O1, Lifemother=O4), subtract the CSV's Overgrowth internals, report the difference (with the order(s) each is reachable at). Related to the reachability check below and could share its appears-at computation.
