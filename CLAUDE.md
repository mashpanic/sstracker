...
- [ ] **`Gluttonous Masses` has two extract layouts; the app only shows one.** R2's Thaddeus battle has two scenarios in `waves.json` — `R2_Battle_TroopBuffFeed` and `R2_Battle_TroopTitanskin` — with **different trash**, but `WAVESET_TO_SCENARIO` maps the single `Gluttonous Masses` wave-set name only to `TroopBuffFeed`. So the app never renders the Titanskin layout. This matters for data: **base `Fleshfruit` at O3 appears only in `TroopTitanskin`** (BuffFeed has upgraded to `Mother's Fleshfruit` by O3) — Titanskin O3 wave 4 = `Mother's Zealot, Mother's Glutmass, Fleshfruit`. **The layout is independent of the boss** (video confirms Quoto at O3 ran the *BuffFeed* layout), so it **cannot** be tied to the selected battle variant. **Open question (watching, 2026-06-15):** `TroopTitanskin` may be **deprecated / never spawned** — present in the data but unused, like the `Mother's Spearman` / `Mother's Terrifying Amalgam` roster-only enemies. So far every observed R2 battle (incl. Quoto) has been the BuffFeed layout. **If Titanskin is confirmed unused:** base `Fleshfruit` O3 is uncollectable → mark its CSV cell `-`, the app showing only BuffFeed is correct (close this TODO, no UI change). **If Titanskin does appear:** surface both layouts (sub-layout toggle for Gluttonous Masses, or list both). (Same two-extracts-per-wave-set shape; also check whether other wave sets have a hidden second layout.)
