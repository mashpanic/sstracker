...
- [ ] **Collect descriptions for 3 minor bosses (currently the "… information" placeholder).** These have empty `''` values in `variantDescriptions` (gamefacts.js), so the info box shows e.g. "Quoto the Destroyer information" instead of a mechanic blurb: **Quoto the Destroyer** (R2 Thaddeus), **Qel the Malaiser** & **Ajax the Deathbringer** (R4 Lylith). Done: Maera's Athane/Korin; R3 **Phalanx** = "Spikes 8. Strike: Lose 1 Spike."; R2 **Elebor** = "Resolve: Gain Titanskin 5."; R3 **The Undying Spirit** = "Reanimate 6. Infested 6. Strike: Gain Infested 2." (2026-06-23). **Action:** observe each minor boss's ability/mechanic in-game (wiki pages not yet written) and fill the `variantDescriptions` strings — hand-authored, same style as the others (e.g. "Applies Rage 1 to enemy units on Incant.").
...
- [ ] **Thaddeus has TWO battle wave sets; the app only lists one.** R2's Thaddeus battle has two scenarios in `waves.json` — `R2_Battle_TroopBuffFeed` and `R2_Battle_TroopTitanskin` — with **different trash**. Their actual in-game names (resolved from each scenario's `battleNameKey` via the Localizer, 2026-06-22) are **different wave sets**, *not* two layouts of one:
  - `TroopBuffFeed` = **"Gluttonous Masses"** (Glutmass + Mother's Hunter swarms + double Cherubs)
  - `TroopTitanskin` = **"Forbidden Fruit"** (Mother's Zealot/Knight + Fleshfruit-heavy)

  But `WAVE_SET_OPTIONS['thaddeus']` lists only `Gluttonous Masses`, so **Forbidden Fruit is never selectable** and Thaddeus is wrongly treated as single-wave-set (its auto-select fires). This **refutes** both the old "Gluttonous Masses covers both R2 scenarios" note and the "Titanskin may be deprecated" guess — Forbidden Fruit is a real, named wave set, just missing from the app. **Data consequence:** base `Fleshfruit` at O3 lives in Forbidden Fruit (Titanskin), so it IS collectable — `Forbidden Fruit` O3 wave 4 = `Mother's Zealot, Mother's Glutmass, Fleshfruit`.

  **Watching (2026-06-22):** user has never noticed "Forbidden Fruit" as the Thaddeus battle name; will watch the battle's name banner (easier than reading waves) to confirm it actually spawns. **If it appears → FIX:** add `Forbidden Fruit` to `WAVE_SET_OPTIONS['thaddeus']`, map `'Forbidden Fruit': 'SoulSavior_R2_Battle_TroopTitanskin'` in `WAVESET_TO_SCENARIO`, re-emit (gives it its own `WAVE_SET_DESCRIPTIONS` entry), and Thaddeus auto-select stops (two options). **If it truly never appears →** Forbidden Fruit is unused; mark Fleshfruit O3 `-`. Also: the wiki/`battleNameKey` names match the others (Harassing Snipers, Rabble-Rousers verified) — so the friendly names *are* extractable; consider sourcing `WAVE_SET_OPTIONS` from `battleNameKey` instead of hand-authoring.

  **How to tell the two layouts apart in-game (Wave ① of the Thaddeus battle):** BuffFeed opens with 2× Glutmass + 2× Cherub and *no Zephyrite/Zealot*; Titanskin always has a **Mother's Zealot** in wave ① (and a Fleshfruit instead of the 2nd Glutmass). Quick rule: **Zealot in the opening wave → Titanskin** (the layout with base Fleshfruit O3); **2 Glutmass + 2 Cherubs, no Zealot → BuffFeed** (skip). Watch this to resolve the deprecated-or-not question. Full Wave ① by order:
