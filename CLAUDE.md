...
`mt2_emit_boss_stats.py` emit Overgrowth into `gamefacts.js` — the observed-stats
section; moved from `gamedata.js` 2026-07-18). **Observations,
...
| `R4` Lylith | Qel the Malaiser · Ajax the Deathbringer | **Lylith**: Plaguebringer · Energy Vampire · Innoculation |
...
**Note-text punctuation convention (game-knowledge formatting, 2026-07-04).** A note is a `<br>`-joined list of segments. Punctuate each segment by what it is:
- **Bare status keyword** (a game keyword + optional quantity — e.g. `Haste`, `Trample`, `Sniper`, `Infested 2`, `Titanskin 10`, `Armor 9`, `Spikes 8`, `Buffet 3`, `Stealth 1`, `Spell Shield 1`) → **no trailing period**, in *any* position (prefix or standalone). Stack multiples with `<br>`, not inline periods: `Sap 9<br>Infested 6` (not `Sap 9. Infested 6.`).
- **Trigger/effect clause** (a `Trigger:` with a colon — `Strike:`, `Resolve:`, `Revenge:`, `Gorge:`, `Eaten:`, `Enchant:`, `Incant:`, `Shift:`, `Hellborne Harvest:`, `On Team Turn:`, …) *or* any sentence with a verb (e.g. Jeermask's `Apply Dazed 2 to the attacker.`) → **ends with a period**.

So: **period iff the segment is a clause; never on a bare status.** Example combining both: `Haste<br>Enchant: Grant enemy units Haste.` · `Spikes 8<br>Strike: Lose Spikes 1.` · `Titanskin 10<br>Revenge: Lose Titanskin 1 and Gain Rage 2.` The rule is a pure string transform (no game data needed); if a bulk re-normalization is ever wanted, classify each `<br>` segment as bare-status (only keyword+quantity tokens) vs clause and punctuate accordingly. Applied across the CSV 2026-07-04 (16 notes fixed; `mt2_collect_observations.py`'s note prompts are free-form, so re-check new entries against this).
...
**O1–O5** = Oversoul rings (combat encounter order), not region visits. Astrael only appears at O1; the four mid regions take O1–O4 (your routing choice); **Lifemother is its own tier, O5** (added 2026-07-08 — see the O5 note below). (Historically Lifemother was folded into O4; observed status-effect scaling beyond O4 forced the split.)

**O5 (Lifemother) — added 2026-07-08.** Lifemother is now collected and displayed at **order 5**, a distinct scaling tier above the four mid-run rings. Its ATK/HP are believed equal to O4 (seeded as copies), but status-effect *amounts* in the notes scale further, so O5 notes are re-observed in-game. Data-model impact: the 13 Lifemother-wave enemies carry `order=5` rows in `difficulty_observations.csv` (copies of O4, `verified=No`); the 3 Lifemother bosses were relabeled O4→O5 (they're order-invariant single strings in `BOSS_STATS`, so their emitted values are unchanged). `ENEMY_STATS`/`ENEMY_NOTES` arrays are now `[O1..O5]` (5 elements; non-Lifemother enemies get `null` at O5). `BOSS_STATS` mid-region arrays stay `[O1..O4]` (no boss is looked up at O5). `encounterOrder('lifemother')` returns 5 in `app.js`; the collector's Lifemother node is `fixed_order=5` and `collect_battle`/`reachable_cells` clamp the O5 lookup to the top wave tier (waves.json only encodes tiers 0–3, T1–T4).
...
- [x] **Variant↔scenario pairing — RESOLVED for all boss trios.** **Rule: the `BossBattle` suffix tag = the named boss's *own* combat mechanic** (not its defeat mutator), confirmed by the final-wave boss `CharacterData` internal name.
  ...
  - **R2 Thaddeus / R4 Lylith — RESOLVED via boss internal name (2026-06-14).** The numbered *section* suffix (`FavoredChild_{1,2,3}`, `EstrangedChild_{1,2,3}`) is opaque, but the final-wave boss `CharacterData` internal name encodes the mechanic and matches the wiki 1:1 (read from `waves.json`). Numbering does **not** follow the wiki listing order.
    ...
    - **R4:** `EstrangedChild_1` = `…Spurned_DualismAt50` → **Plaguebringer** · `EstrangedChild_2` = `…_ArmorPerDebuff` → **Innoculation** · `EstrangedChild_3` = `…_WitherbloomOnAction` → **Energy Vampire**.
...
- [x] **Minor-boss HP formula is wrong — CLOSED as moot (2026-07-04).** The bad formula lived in `compute_orders`/`boss_overgrowth_scaled` (over-inflated the small-base minor "TrainBoss" bosses via HP constants that assume region-boss base HP). That whole computed path was **retired to `deprecated/mt2_build_outputs.py`** and the `--seed-bosses-from-computed` bridge removed, so nothing emits those wrong HPs anymore — `BOSS_STATS` is fully observed from `difficulty_observations.csv`. Any remaining wrong seeds are overwritten as the minor TrainBosses get observed (in progress). Fix the formula only if the computed path is ever revived; not otherwise worth it. (History: Athane base 5/400 → computed O3 36⚔️/4667❤️ vs observed 44⚔️/2908❤️.)
  - [x] **Check Quoto the Destroyer @ 37/4517 — CLOSED as moot (2026-07-04).** Was a data point for the minor-boss formula (now retired, see parent). Quoto is observed anyway — O1 `12/528` verified this run; O3 `40/4519` just needs one in-game confirm if ever desired. Not worth tracking.
...
- [x] **`ENEMY_STATS` emitted — DONE** via `extraction/mt2_emit_enemy_stats.py --write gamefacts.js` (block moved from `gamedata.js` to `gamefacts.js` 2026-07-18). Keyed by display name → `[O1,O2,O3,O4]` of `'ATK⚔️ HP❤️'`, `null` per unobserved order. Same string/array shape as `BOSS_STATS` (so `pickByOrder` semantics apply). **Re-run after editing the CSV.**
- [x] **Hover wired in `app.js` — DONE.** `wrapEnemyStats(html, order)` wraps recognized enemy names in the info-box wave lists in a `<span class="enemy-stat" title="…">` (native tooltip, dotted underline styled in `index.html`). Order resolved by `encounterOrder(key)` (astrael=O1, lifemother=O5, mid-run = region Order dropdown). Unobserved/no-order → title says "not yet recorded". Popup deliberately uses the native `title` (deferred styled-tooltip option not taken).
...
- [x] **Sanity-check the observation data (validation pass) — DONE (2026-07-04).** Implemented as `mt2_collect_observations.py --check`, covering the monotonic-by-order and NOTE-divergence checks (plus the `mt2_audit_coverage.py` reachability cross-check). Runs clean: **0 MONO** (the lone flag, Bloom Mother's Blade ATK O2=8→O3=5, was a typo — O2 corrected to 5), and **24 NOTE** which are all *intentional* per-order divergences, not errors. Remaining sub-checks from the original plan (paired cells, valid-contents, plausible-ramp, reviewed-consistency) can be added to `--check` if a future need arises, but the primary structural checks are in place.
...
- `gamedata.js` — **pipeline-generated**: `BOSS_WAVE_DESCRIPTIONS` + `WAVE_SET_DESCRIPTIONS` (by `mt2_emit_wave_descriptions.py`)
- `gamefacts.js` — **human-established facts**, NOT extractable. Hand-authored wiki prose: `MUTATORS`, `variantDescriptions`, `VARIANT_OPTIONS`, `WAVE_SET_OPTIONS`, plus `CHAMPION_CLANS` (24 champions × 12 clans) and `PYRE_HEARTS` — the pipeline must never write the prose. Also holds the in-game **observed blocks** `BOSS_STATS` (by `mt2_emit_boss_stats.py`), `ENEMY_STATS` (by `mt2_emit_enemy_stats.py`), and `ENEMY_NOTES` (by `mt2_emit_notes.py`), moved here from `gamedata.js` 2026-07-18 because they're hand-observed, not extracted; those emitters rewrite ONLY their own const block (plus its leading `//` comment lines) — the one sanctioned machine write to this file.
- `app.js` — tracker logic (all functions) + the Win-Loss (champion win/loss) record + screen-switching logic + UI config (`CENTRAL_NODE_OPTIONS`, `TRACK_NODE_OPTIONS`, `PATH_TRACK_OPTIONS`, `ORDER_OPTIONS`, `encounterInfo`)

**Verifying changes in a browser: use Safari, not Chrome.** Past work here used
headless Chrome (`--headless=new --dump-dom`) to check rendered output; that
permission was removed 2026-09-14 in favor of the built-in Safari MCP server
(`safari-mcp` — see the global `~/.claude/CLAUDE.md` "Browser automation"
section). Use `navigate_to_url`/`get_page_content`/`evaluate_javascript`/
`screenshot` against `file:///Users/klara/Claude/Projects/sstracker/index.html`
for DOM/behavior checks, or plain `open -a Safari` for an eyeball check —
don't reach for Chrome or reintroduce a headless-Chrome permission for this.

**Three-screen layout (2026-07-17).** The product shows **one screen at a time** — Run Tracker, Win-Loss, Help — switched by `showScreen()` buttons at the bottom of each screen; `pickInitialScreen()` opens Help for first-time visitors (empty `localStorage`), else the Run Tracker. The **Win-Loss** screen (internal id `#screen-champions`) is a per-champion win/loss tally (two-column grid, drag-reorder, click-to-select with a bottom W+/W−/L+/L− editor) built from `CHAMPION_CLANS`, persisted under `mt2-champion-records-v1`, fully independent of the run grid. See `docs/champion_record.md`.
