...
- [ ] **`(Bonus)` wave groups — tier gating unverified.** 8 scenarios are still flat across O1–O4 (R1 battles `AscendAttacker`/`HealOnShiftHeavy`, all 3 R1 Maera bosses `DutifulChild_*`, all 3 R3 Tivi bosses). Their gated-looking groups carry a `(Bonus)` label with **no number**, so `wave_min_tier` can't infer a min tier and leaves them always-present (possibly correct, possibly the same over-count bug). **Action:** check one in-game (e.g. a Maera or Tivi boss at O1 — does it have 6 waves or fewer?). If `(Bonus)` waves are tier-gated, find the numeric signal and add it to `wave_min_tier`; if they're genuinely always-present, no change needed.
