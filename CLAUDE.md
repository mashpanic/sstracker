...
- [ ] **`Gluttonous Masses` has two extract layouts; the app only shows one.** R2's Thaddeus battle has two scenarios in `waves.json` — `R2_Battle_TroopBuffFeed` and `R2_Battle_TroopTitanskin` — with **different trash**, but `WAVESET_TO_SCENARIO` maps the single `Gluttonous Masses` wave-set name only to `TroopBuffFeed`. So the app never renders the Titanskin layout. This matters for data: **base `Fleshfruit` at O3 appears only in `TroopTitanskin`** (BuffFeed has upgraded to `Mother's Fleshfruit` by O3) — Titanskin O3 wave 4 = `Mother's Zealot, Mother's Glutmass, Fleshfruit`. `TroopTitanskin` is very likely the **Quoto** encounter (Quoto's internal is `…_Titanskin`). **Action:** decide how to surface both — e.g. tie the displayed Thaddeus wave set to the selected battle variant (Elebor→BuffFeed, Quoto→Titanskin), or give Gluttonous Masses two sub-layouts. (This is the same two-extracts-per-wave-set shape; check whether other wave sets have a hidden second layout too.)
