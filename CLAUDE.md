...
mt2_emit_notes.py          → rewrites the ENEMY_NOTES block in gamedata.js from difficulty_observations.csv (note column; enemies AND bosses, keyed by display name; difficulty/order-invariant). Feeds the app's enemy/boss ability-note hover/click popover. Re-run after editing notes in the CSV.
mt2_collect_observations.py → guided TUI collector → difficulty_observations.csv (Bloom/Tangle/Overgrowth enemy AND boss ATK/HP + notes + verified). Entering a stat in the run-walk sets verified=Yes (the act of observing in-game); bulk/seed modes write verified=No. The walk keys its ✓/remaining on verified (not just on having stats), so the marks are: ✓ verified · ~ provisional (seeded ATK/HP, unconfirmed — shown "(unverified)") · blank unrecorded. Pressing Enter on a ~ row accepts its prefilled value and flips it to ✓. The interactive main menu offers **Walk a run** (collect in visit order) and **Manual collection** (jump straight to a chosen encounter: pick a region/Lifemother 1-5, then its battle/boss 1-2 or Lifemother variant 1-3, then an order — loops back to the region menu; Astrael isn't offered there, collect it via the walk). Maintenance modes: --check (validator, incl. NOTE divergence), --prefill-base DIFF (seed a difficulty's non-boss cells from roster base, verified=No), --tidy-notes (propagate each enemy's note across its orders/difficulties), --seed-notes SRC DST (copy SRC's notes onto DST as notes-only rows with blank ATK/HP — e.g. `--seed-notes Bloom Tangle` so a fresh Tangle run shows each enemy's note for inline editing), --seed-bosses-from-computed (bridge-seed 84 Overgrowth boss rows from the computed BOSS_STATS in gamedata.js, verified=No, to be observed-and-overwritten). See its module docstring.
...
**The `note` column (ability text) is assumed difficulty- and order-invariant — working assumption, not fully confirmed.** Each enemy's `note` (its ability/mechanic blurb, `<br>`-joined) is hand-authored and believed identical across all of that enemy's rows (every Bloom/Tangle/Overgrowth × O1–O4 row for a given `internal` carries the same text). This is what `--tidy-notes` propagation and `--seed-notes` copying rely on, and what lets a note edited on one row be replicated to the rest. **Status:** as of 2026-06-28 the file is internally consistent — every `internal` has exactly one distinct note (verified by grouping the `note` column by `internal`); the lone prior exception, **Insatiable**, had a stray missing `Trample` on its O3 rows and was normalized. **Not yet confirmed in-game** that abilities never genuinely change with order/difficulty — the user is spot-checking live, with **Lifemother and the major/region bosses the most likely place for a real per-order/per-difficulty exception** (their mechanics are the most elaborate). If a genuine variation is found, the invariance assumption breaks for that enemy and the propagation tooling must not flatten it — record the per-(order/difficulty) text deliberately. Re-check internal consistency with: group the `note` column by `internal`, flag any `internal` with >1 distinct non-empty note.
