...
- `app.js` — tracker logic (all functions) + the Champion Record + screen-switching logic + UI config (`CENTRAL_NODE_OPTIONS`, `TRACK_NODE_OPTIONS`, `PATH_TRACK_OPTIONS`, `ORDER_OPTIONS`, `encounterInfo`)

**Three-screen layout (2026-07-17).** The product shows **one screen at a time** — Run Tracker (screen 1), Champion Record (screen 2), Help (screen 3) — switched by `showScreen()` buttons at the bottom of each screen; `pickInitialScreen()` opens Help for first-time visitors (empty `localStorage`), else the Run Tracker. The **Champion Record** is a per-champion Win/Loss tally (two-column grid, drag-reorder, click-to-highlight) built from `CHAMPION_CLANS`, persisted under `mt2-champion-records-v1`, fully independent of the run grid. See `docs/champion_record.md`.
